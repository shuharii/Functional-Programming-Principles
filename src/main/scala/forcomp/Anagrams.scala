...
object Anagrams extends AnagramsInterface:
  ...
  def wordOccurrences(w: Word): Occurrences =
    for (x <- w.toLowerCase.toList.groupBy((c: Char) => c).toList.sortBy(_._1))
      yield (x._1, x._2.length)
  ...
  def sentenceAnagrams(sentence: Sentence): List[Sentence] = {
    def anagramer(occurrences: Occurrences): List[Sentence] = {
      if (occurrences.isEmpty) List(List())
      else for {
        com <- combinations(occurrences)
        word <- dictionaryByOccurrences(com)
        remain <- anagramer(subtract(occurrences, com))
      } yield word :: remain
    }
    anagramer(sentenceOccurrences(sentence))
  }
